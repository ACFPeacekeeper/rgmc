...
cur_dir="/home/pkhunter/Repositories/rmgm/rmgm_code"

def process_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="rmgm", description="Test performance and robustness of several models")
    ...
    parser.add_argument('-s', '--stage', type=str, default='train', choices=['train', 'test_model', 'test_classifier', 'all'], help='Stage(s) of the pipeline to execute in the experiment.')
    ...
    if args.stage == 'train' or args.stage == 'all':
        if args.epochs < 1:
            parser.error("Argument error: number of epochs must be a positive and non-zero integer.")
        elif args.batch_size < 1:
            parser.error("Argument error: batch_size value must be a positive and non-zero integer.")
        elif args.latent_dim < 1:
            parser.error("Argument error: latent_dim value must be a positive and non-zero integer.")
        elif args.checkpoint < 1:
            parser.error("Argument error: checkpoint value must be a positive and non-zero integer.")
        elif args.checkpoint > args.epochs:
            parser.error("Argument error: checkpoint value must be smaller than or equal to the number of epochs.")
...
def train_model(arguments):
    if arguments.model_type.upper() == 'VAE':
        model = vae.VAE(arguments.latent_dim)
    ...
    for epoch in tqdm(range(arguments.epochs)):
        print(f'Epoch {epoch}')
        for img_sample, traj_sample in tqdm(zip(img_samples, traj_samples), total=len(traj_samples)):
        ...
        if checkpoint_counter == 0:
            ...
            torch.save(model.state_dict(), os.path.join(cur_dir, "checkpoints", f'{arguments.model_file}_{epoch}'))
            checkpoint_counter = arguments.checkpoint

    print("Saving per epoch loss values to file...")
    with open(os.path.join(cur_dir, "results.txt"), 'w') as file:
